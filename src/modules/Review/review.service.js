...
async function getAverageReviews(filters) {
  ...
  const fourStarReviews = await Review.countDocuments({
    ...filters,
    rating: 4,
  });
  const threeStarReviews = await Review.countDocuments({
    ...filters,
    rating: 3,
  });
...
const addReview = async (reviewBody) => {
  ...
  const dispatchCompleted =
    driverDetails.dispatchCompleted > 0 ? driverDetails.dispatchCompleted : 1;
  if (!driverDetails) {
  ...
  if (!existingReview) {
    ...
    driverDetails.ratings =
      (driverDetails.ratings + reviewBody.rating) / (dispatchCompleted + 1);
  } else {
    driverDetails.ratings =
      (driverDetails.ratings - existingReview.rating + reviewBody.rating) /
      dispatchCompleted;
...
const getTopReviewOfootms = async (ootmsId) => {
  ...
  const topReview = await Review.findOne({ ootms: ootmsId })
    .sort({ rating: -1 })
    .populate("user", "fullName image");
