...
class DashboardService {
    ...
    async getRevenueAnalytics(year?: number): Promise<IRevenueAnalytics[]> {
        ...
        const revenueStats = bookings.reduce((monthlyRevenue, booking) => {
    ...
    private async getTotalRevenue(fromDate?: Date): Promise<number> {
        ...
        if (fromDate) {
            ...
            booking = await Booking.find({ paymentStatus: PAYMENT_STATUS.PAID, status: BOOKING_STATUS.COMPLETED, createdAt: { $gte: fromDate } }).select('finalAmount adminRevenuePercent createdAt').lean();
        } else {
    ...
    async getCustomers(query: Record<string, any>) {
        ...
        const customersWithTotalAmount = await Promise.all(result.map(async (customer) => {
    ...
    async getServiceProviders(query: Record<string, any>) {
        ...
        const serviceProvidersWithTotalEarn = await Promise.all(result.map(async (serviceProvider) => {
