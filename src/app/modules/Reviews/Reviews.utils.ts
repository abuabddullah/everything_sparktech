...
const updateAvgRatingOfRefference = async (
  ...
  const totalRating = refferenceWithReviews.reviews.reduce(
    (sum: any, review: Partial<IReviews>) => sum + (review as any).rating,
    0,
  )
  ...
  await mongoose
    .model(type)
    .findByIdAndUpdate(refferenceId, { avgRating, reviewsCount })
}

// Helper to update user avgRating from review doc
export async function updateRefferenceAvgRatingFromReview(
  doc: Partial<IReviews>,
) {
  if (!doc) return
  await updateAvgRatingOfRefference(
    doc.type as ReviewsType,
    doc.refferenceId as Types.ObjectId,
  )
}
