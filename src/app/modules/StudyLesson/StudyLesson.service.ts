...
const createStudyLesson = async (payload: IStudyLesson): Promise<IStudyLesson> => {
     const result = await StudyLesson.create(payload);
     return result;
};

const getAllStudyLessons = async (query: Record<string, any>): Promise<{ meta: { total: number; page: number; limit: number; }; result: IStudyLesson[]; }> => {
...
const updateStudyLesson = async (id: string, payload: Partial<IStudyLesson>): Promise<IStudyLesson | null> => {
...
const hardDeleteStudyLesson = async (id: string): Promise<IStudyLesson | null> => {
